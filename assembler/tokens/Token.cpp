...
std::ostream& operator<<(std::ostream& os, const Token& token) {
    ...
    if (type_id == WordToken::TYPE_ID) {
        WordToken word_token = *dynamic_cast<const WordToken*>(&token);
//        WordToken word_token = *static_cast<const WordToken*>(&token);
        os << "WORD: " << word_token.get_word();
    } else if (type_id == RegisterToken::TYPE_ID) {
        RegisterToken register_token = *dynamic_cast<const RegisterToken*>(&token);
        os << "REGISTER: " << register_token.get_reg();
    } else if (type_id == NumberToken::TYPE_ID) {
        NumberToken number_token = *dynamic_cast<const NumberToken*>(&token);
        os << "NUMBER: " << number_token.get_num();
    } else if (type_id == InstructionToken::TYPE_ID) {
        InstructionToken instruction_token = *dynamic_cast<const InstructionToken*>(&token);
        os << "INSTRUCTION: " << instruction_token.get_instr_name();
    } else if (type_id == LabelToken::TYPE_ID) {
        LabelToken label_token = *dynamic_cast<const LabelToken*>(&token);
